...
export class GoldenMasterExtractor {
  ...
  private calculatePokemonStats(pokemon: Pokemon): any {
    ...
    const stats: PokemonStats = {
      ...
      spAttack: this.calculateStat(pokemon.baseStats.spAttack, ivs.spAttack, evs.spAttack, level, natureModifiers.spAttack),
      spDefense: this.calculateStat(pokemon.baseStats.spDefense, ivs.spDefense, evs.spDefense, level, natureModifiers.spDefense),
  ...
  private calculateCaptureRates(inputGameState: any): any {
    ...
    for (const pokeball of pokeballs) {
      ...
      const captureValue = ((3 * wildPokemon.maxHp - 2 * wildPokemon.currentHp) * baseCatchRate * ballModifier * statusBonus) / (3 * wildPokemon.maxHp);
