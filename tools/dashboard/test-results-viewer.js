...
class TestResultsViewer {
  ...
  renderTestSummary() {
    ...
    this.updateSummaryCard(
      ...
      `${passRate}%`,
      "📊 Pass Rate",
      passRate >= 90 ? "success" : passRate >= 70 ? "warning" : "danger",
    );
  ...
  updateTrendIndicators(summary) {
    ...
    indicators.forEach(indicator => {
      ...
      if (!element || !summary.trend[indicator]) {
        return;
      }
  ...
  renderTestTable() {
    ...
    if (!tableBody || !this.data || !this.data.suites) {
      return;
    }
  ...
  renderTestTrends() {
    ...
    if (!canvas || !this.data || !this.data.trends) {
      return;
    }
