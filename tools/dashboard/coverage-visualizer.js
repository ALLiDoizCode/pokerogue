...
class CoverageVisualizer {
    ...
    renderFileHeatmap() {
        ...
        if (!container || !this.data || !this.data.files) return;
    ...
    renderTrendChart() {
        ...
        if (!canvas || !this.data || !this.data.history) return;
    ...
    renderCoverageBreakdown() {
        ...
        if (!canvas || !this.data || !this.data.breakdown) return;
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: buckets.map((_, i) => {
                        if (i >= 9) return this.config.colors.high;
                        if (i >= 7) return this.config.colors.medium;
    ...
    renderLineDetails(details) {
        if (!details.lines || details.lines.length === 0) return '';
    ...
    getCoverageClass(coverage) {
        if (coverage >= this.config.thresholds.high) return 'high';
        if (coverage >= this.config.thresholds.medium) return 'medium';
        return 'low';
    }
    ...
    convertToCSV() {
        ...
        const headers = ['File Path', 'Line Coverage %', 'Lines Covered', 'Total Lines', 'Function Coverage %', 'Functions Covered', 'Total Functions'];
