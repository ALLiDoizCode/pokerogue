...
class TestIssueMapper {
        ...
        if (!testCase) {
            ...
            for (const [key, issueNumbers] of Object.entries(this.mapping.testToIssues)) {
                if (key.startsWith(testFile + ':')) {
                    matchingIssues.push(...issueNumbers.map(issueNumber => ({
                        issueNumber,
                        ...this.mapping.issueToTests[issueNumber]
                    })));
                }
    ...
    processTestResults(testResults) {
        ...
        for (const testResult of testResults) {
            if (testResult.status === 'failed') {
                ...
                const existingIssues = this.findIssuesForTest(
                    testResult.testFile, 
                    testResult.testCase
                );
    ...
    getTestKeyFromIssueData(issueData) {
        return issueData.testCase ? 
            `${issueData.testFile}:${issueData.testCase}` : 
            issueData.testFile;
    }
    ...
    convertToCSV() {
        const headers = [
            'Issue Number', 'Test File', 'Test Case', 'Test Suite', 
            'Status', 'Failure Type', 'Priority', 'Created At', 
            'Last Updated', 'Resolved At', 'URL'
        ];
        ...
        for (const [issueNumber, issueData] of Object.entries(this.mapping.issueToTests)) {
            if (issueData.status === 'closed' && issueData.closedAt) {
                ...
                if (closedDate < cutoffDate) {
                    ...
                    const testKey = this.getTestKeyFromIssueData(issueData);
                    if (this.mapping.testToIssues[testKey]) {
                        this.mapping.testToIssues[testKey] = this.mapping.testToIssues[testKey]
                            .filter(num => num !== parseInt(issueNumber));
...
if (require.main === module) {
    ...
    if (command === 'map' && args.length >= 3) {
        ...
        const issueNumber = parseInt(args[2]);
