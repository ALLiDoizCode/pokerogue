...
const _EVOLUTIONS_FILE = path.join(TYPESCRIPT_BASE, "data/balance/pokemon-evolutions.ts");
const _LEVEL_MOVES_FILE = path.join(TYPESCRIPT_BASE, "data/balance/pokemon-level-moves.ts");
...
const ABILITIES = {
  "AbilityId.NONE": 0,
  "AbilityId.OVERGROW": 65,
  "AbilityId.BLAZE": 66,
  "AbilityId.TORRENT": 67,
  "AbilityId.SWARM": 68,
  "AbilityId.CHLOROPHYLL": 34,
  "AbilityId.SOLAR_POWER": 94,
  "AbilityId.RAIN_DISH": 44,
  "AbilityId.STATIC": 9,
  "AbilityId.LIGHTNING_ROD": 31,
  "AbilityId.PRESSURE": 46,
  "AbilityId.UNNERVE": 186,
  "AbilityId.THICK_FAT": 47,
  "AbilityId.DROUGHT": 70,
  "AbilityId.TOUGH_CLAWS": 181,
};
...
function parseSpeciesIds() {
  ...
  for (const line of lines) {
    ...
    if (explicitValueMatch) {
      ...
      currentId = Number.parseInt(value);
...
function parseSpeciesData() {
  ...
  const speciesPattern =
    /new PokemonSpecies\(\s*SpeciesId\.(\w+),\s*(\d+),\s*([^,]+),\s*([^,]+),\s*([^,]+),\s*"([^"]+)",\s*PokemonType\.(\w+),\s*(?:PokemonType\.(\w+)|null),\s*([\d.]+),\s*([\d.]+),\s*AbilityId\.(\w+),\s*AbilityId\.(\w+),\s*AbilityId\.(\w+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+),\s*(\d+)/g;

  let match;
  let count = 0;

  while ((match = speciesPattern.exec(content)) !== null && count < 1000) {
    // Limit for safety
    const [
      ,
      speciesName,
    ...
    species.push({
      ...
      baseStats: [
        ...
        Number.parseInt(atk),
        Number.parseInt(def),
        Number.parseInt(spatk),
        Number.parseInt(spdef),
        Number.parseInt(spd),
      ],
      baseTotal: Number.parseInt(baseTotal),
      catchRate: Number.parseInt(catchRate),
      baseFriendship: Number.parseInt(baseFriendship),
      baseExp: Number.parseInt(baseExp),
    });
...
function generateLuaData(speciesIds, speciesData) {
  ...
  for (const [name, id] of sortedSpecies.slice(0, 50)) {
    // Limit for initial migration
  ...
  for (let i = 0; i < Math.min(speciesData.length, 50); i++) {
    // Limit for initial migration
    ...
    luaLines.push(
      `        h = ${Math.round(species.height * 10)}, w = ${Math.round(species.weight * 10)}, -- height (dm), weight (hg)`,
    );
    luaLines.push(
      `        cr = ${species.catchRate}, bf = ${species.baseFriendship}, be = ${species.baseExp}, -- catch rate, base friendship, base exp`,
    );
