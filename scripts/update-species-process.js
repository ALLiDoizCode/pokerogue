...
let processContent = fs.readFileSync(PROCESS_FILE, 'utf8');

// Replace the SpeciesDatabase section
const processDbPattern = /(-- Embedded Species Database.*?\nlocal SpeciesDatabase = \{)([\s\S]*?)(\}[\s\S]*?-- Create optimized indexes)/;
...
const updatedProcessContent = processContent.replace(
    processDbPattern, 
    `$1${speciesDatabaseContent}$3`
);
