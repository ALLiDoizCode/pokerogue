    ...
    for (const pattern of patterns) {
      ...
      if (match) {
        if (match.length > 20) {
          ...
          species.generation = Number.parseInt(generation);
          ...
          const name = match[1];
          const generation = match[2] ? Number.parseInt(match[2]) : 1;
...
function generateLuaFromSpecies(species) {
  ...
  species.forEach(pokemon => {
    ...
    const typesArray = (pokemon.types || ["NORMAL"]).length === 1 ? `{${types}}` : `{${types}}`;
    ...
    const baseStats = [
      pokemon.baseStats?.hp || 50,
      pokemon.baseStats?.attack || 50,
      pokemon.baseStats?.defense || 50,
      pokemon.baseStats?.specialAttack || 50,
      pokemon.baseStats?.specialDefense || 50,
      pokemon.baseStats?.speed || 50,
    ].join(", ");
    ...
    const luaEntry = `    [${pokemon.id}] = {
        ...
        h = ${Math.round((pokemon.physical?.height || 1.0) * 10)}, w = ${Math.round((pokemon.physical?.weight || 10.0) * 10)}, -- height (dm), weight (hg)
...
${Object.keys(chunks)
  .map(gen => `- Gen ${gen}: ${chunks[gen].length} species`)
  .join("\n")}
