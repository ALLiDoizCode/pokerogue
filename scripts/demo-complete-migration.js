...
function demonstrateArchitecture(manifest) {
    ...
    manifest.chunks.forEach(chunk => {
        ...
        console.log(`   ${exists ? '✅' : '❌'} Gen ${chunk.generation}: ${chunk.speciesCount} species (${Math.round(chunk.size/1024)}KB)`);
    });
    console.log();
}

function demonstrateCapabilities(manifest) {
...
function showImprovementSummary(originalCount, manifest) {
    ...
    console.log(`   Coverage: ${Math.round(originalCount/1082*100)}% → 100% (+${coverageImprovement}% improvement)`);
