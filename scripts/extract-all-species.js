...
function parseTypeScriptSpecies(content) {
  ...
  pushMatches.forEach((pushContent, index) => {
    if (index === 0) {
      return; // Skip the first empty split
    }
    ...
    try {
      ...
      if (match) {
        ...
        const speciesData = {
          ...
          baseStats: {
            hp: Number.parseInt(baseHp),
            attack: Number.parseInt(baseAtk),
            defense: Number.parseInt(baseDef),
            specialAttack: Number.parseInt(baseSpatk),
            specialDefense: Number.parseInt(baseSpdef),
            speed: Number.parseInt(baseSpd),
          },
          abilities: {
            primary: ability1,
          ...
          characteristics: {
            ...
            genderRatio: Number.parseFloat(genderRatio),
...
function generateLuaSpeciesData(species) {
  ...
  species.forEach((pokemon, _index) => {
    const types = pokemon.types.map(t => `POKEMON_TYPE.${t}`).join(", ");
    const typesArray = pokemon.types.length === 1 ? `{${types}}` : `{${types}}`;
...
return Gen${generation}Species`;
    ...
    console.log(
      `  Generated Gen ${generation}: ${chunkFile} (${Math.round(fileSize / 1024)}KB, ${species.length} species)`,
    );

    if (fileSize > CONFIG.maxChunkSize) {
      console.warn(
        `  WARNING: Gen ${generation} chunk (${Math.round(fileSize / 1024)}KB) exceeds limit (${Math.round(CONFIG.maxChunkSize / 1024)}KB)`,
      );
    }
...
function generateManifest(chunkFiles, totalSpecies) {
  ...
  const manifest = {
    ...
    chunks: chunkFiles.map(chunk => ({
      generation: Number.parseInt(chunk.generation),
...
${manifest.chunks
  .map(chunk => `- **Gen ${chunk.generation}:** ${chunk.speciesCount} species (${Math.round(chunk.size / 1024)}KB)`)
  .join("\n")}
