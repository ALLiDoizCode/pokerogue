...
describe('Server-Side Bypass Protection', () => {
  ...
  describe('Request Validation', () => {
    ...
    it('should reject requests with stale timestamps', () => {
      ...
      const staleRequest = JSON.stringify({
        ...
        justification: 'This is a detailed justification that meets the minimum length requirement for testing purposes',
    ...
    it('should reject requests with insufficient justification', () => {
      ...
      try {
        execSync(`cd ${TEST_DIR} && node ${VALIDATOR_PATH} validate '${shortJustificationRequest}'`, { encoding: 'utf8' });
  ...
  describe('Admin Approval', () => {
    ...
    it('should accept requests with valid admin approval', () => {
      ...
      const result = execSync(`cd ${TEST_DIR} && node ${VALIDATOR_PATH} validate '${adminApprovedRequest}'`, { encoding: 'utf8' });
  ...
  describe('Emergency Override', () => {
    it('should validate emergency override with correct secret', () => {
      ...
      const result = execSync(`cd ${TEST_DIR} && node ${VALIDATOR_PATH} validate '${emergencyRequest}'`, { encoding: 'utf8' });
      const response = JSON.parse(result);
  ...
  describe('Token Generation and Verification', () => {
    it('should generate valid bypass tokens', () => {
      ...
      const result = execSync(`cd ${TEST_DIR} && node ${VALIDATOR_PATH} validate '${validRequest}'`, { encoding: 'utf8' });
      const response = JSON.parse(result);
      ...
      // Verify the token
      const verifyResult = execSync(`cd ${TEST_DIR} && node ${VALIDATOR_PATH} verify '${response.bypassToken}'`, { encoding: 'utf8' });
  ...
  describe('Security Hardening', () => {
    it('should use timing-safe comparison for secrets', () => {
      ...
      const result = execSync(`cd ${TEST_DIR} && node ${VALIDATOR_PATH} validate '${correctSecretRequest}'`, { encoding: 'utf8' });
      const response = JSON.parse(result);
