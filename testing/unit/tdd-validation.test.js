...
describe('TDD Validation Logic', () => {
  ...
  describe('getTestFilePath functionality', () => {
    it('should map processes files to testing/unit', () => {
      ...
      try {
        const result = execSync(`cd ${TEST_DIR} && lua ${TDD_VALIDATOR_PATH} validate processes/battle.lua`, { encoding: 'utf8' });
    ...
    it('should map ao-processes files correctly', () => {
      ...
      try {
        const result = execSync(`cd ${TEST_DIR} && lua ${TDD_VALIDATOR_PATH} validate ao-processes/stats.lua`, { encoding: 'utf8' });
    ...
    it('should skip ao-processes test files', () => {
      const result = execSync(`lua ${TDD_VALIDATOR_PATH} validate ao-processes/tests/unit/stats.test.lua`, { encoding: 'utf8' });
      expect(result).toContain('File excluded from TDD requirements');
    });
  });
