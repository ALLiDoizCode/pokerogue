...
export class EquivalenceValidator {
    ...
    this.addValidationRule('hp_special_cases', (tsValue, aoValue, context) => {
      ...
      if (context.species === 'Shedinja' || context.speciesId === 292) {
        ...
        return {
          ...
          reason: tsValue !== expectedHP || aoValue !== expectedHP ? 
            `Shedinja HP must always be 1. TS: ${tsValue}, AO: ${aoValue}` : null
        };
      }
  ...
  async validateFormulas(typescriptResult, aoLuaResult, scenario) {
    ...
    if (scenario.category === 'pokemon_mechanics' && scenario.inputGameState?.pokemon) {
      const statFormulaValidation = await this.validateStatCalculationFormula(
        typescriptResult,
        aoLuaResult,
        scenario
      );
  ...
  calculateExpectedStats(pokemon) {
    ...
    for (let i = 0; i < statKeys.length; i++) {
      ...
      const baseStat = Math.floor(((2 * baseStats[statKey] + ivs[statKey] + Math.floor(evs[statKey] / 4)) * level / 100) + 5);
  ...
  requiresExactMatch(path, scenario) {
    ...
    return exactMatchPaths.some(exactPath => path.includes(exactPath)) ||
  ...
  assessNumericSeverity(absoluteDifference, relativeDifference, tolerance) {
    ...
    if (absoluteDifference > tolerance * 10) return 'high';
    if (absoluteDifference > tolerance * 2) return 'medium';
    return 'low';
  }
  ...
  calculatePrecisionScore(comparisonResult) {
    ...
    if (totalComparisons === 0) return 1.0;
  ...
  getValidationStatusMessage(validation) {
    if (validation.isEquivalent) {
      return chalk.green(`✅ Mathematical equivalence validated (precision: ${(validation.precisionScore * 100).toFixed(1)}%)`);
    } else {
      const issues = validation.differences.length + validation.ruleViolations.length;
      return chalk.red(`❌ Mathematical equivalence failed (${issues} issues, precision: ${(validation.precisionScore * 100).toFixed(1)}%)`);
    }
  }
  ...
  getEquivalenceSummary() {
    ...
    const avgPrecision = total > 0 ? 
      this.validationResults.reduce((sum, v) => sum + v.precisionScore, 0) / total : 0;
  ...
  getFormulaValidationSummary() {
    const formulaResults = this.validationResults
      .filter(v => v.formulaValidation)
      .map(v => v.formulaValidation);
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            ${Object.entries(report.ruleViolationAnalysis.violationsByRule).map(([rule, count]) => 
              `<p><strong>${rule}:</strong> ${count} violations</p>`
            ).join('')}
        </div>
