-- ============================================================================
-- Abilities Database Process - Ability data with trigger conditions and effect mechanics for sub-100ms performance
...
local ao = ao or { send = function(msg) return true end, id = "abilities-database" }

-- Embedded template functions
local RATE_LIMIT_MAX = 100
local rateLimitCounters = {}
local function validateInput(message) if type(message) ~= "table" then return false, "Message must be a table" end if not message.Action or type(message.Action) ~= "string" then return false, "Action field is required" end if not message.Data or type(message.Data) ~= "table" then return false, "Data field is required" end if not message.Timestamp or type(message.Timestamp) ~= "number" then return false, "Timestamp field is required" end return true, nil end
local function checkRateLimit(address) local currentTime = os.time() local currentMinute = math.floor(currentTime / 60) if not rateLimitCounters[address] then rateLimitCounters[address] = { minute = currentMinute, count = 0 } end local counter = rateLimitCounters[address] if counter.minute ~= currentMinute then counter.minute = currentMinute; counter.count = 0 end if counter.count >= RATE_LIMIT_MAX then return false, "Rate limit exceeded" end counter.count = counter.count + 1 return true, nil end
local function createSuccessResponse(data, processId) return { Action = "SaveState", Data = data, Timestamp = os.time(), ProcessId = processId or ao.id } end
local function createErrorResponse(errorMessage, processId) return { Action = "SaveState", Error = errorMessage, ProcessId = processId or ao.id, Timestamp = os.time() } end
local function handleMessage(message, processId, queryHandler) local isValid, validationError = validateInput(message) if not isValid then return createErrorResponse(validationError, processId) end local senderAddress = message.From or "unknown" local rateLimitOk, rateLimitError = checkRateLimit(senderAddress) if not rateLimitOk then return createErrorResponse(rateLimitError, processId) end local success, result = pcall(function() return queryHandler(message) end) if success then return createSuccessResponse(result, processId) else return createErrorResponse("Query processing failed: " .. tostring(result), processId) end end
